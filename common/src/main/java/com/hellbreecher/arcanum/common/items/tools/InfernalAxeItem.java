...
public class InfernalAxeItem extends AxeItem {
    ...
    public InfernalAxeItem(Identifier id) {
        super(ArcanumToolMaterials.InfernalTool, 6.0F, -3.1F, new Item.Properties()
    ...
    public boolean isCorrectToolForDrops(ItemStack stack, BlockState state) {
        if (!ArcanumConfig.infernalToolsIgnoreHarvestLevel()) {
            return super.isCorrectToolForDrops(stack, state);
        }
        return isCorrectForTool(state, BlockTags.MINEABLE_WITH_AXE);
    }
    ...
    public static boolean onBlockBreak(net.minecraft.world.level.LevelAccessor eventLevel, BlockPos pos, BlockState state, Player player) {
        if (player == null) {
            return true;
        }

        if (!ArcanumConfig.infernalAutosmeltEnabled()) {
            return true;
        }
        ...
        if (!player.getAbilities().instabuild) {
            int dropMultiplier = ArcanumConfig.infernalAutosmeltDropMultiplier();
            int xpMultiplier = ArcanumConfig.infernalAutosmeltXpMultiplier();
